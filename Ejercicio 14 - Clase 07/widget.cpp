...
{
    ui->setupUi(this);
    this->setdb("../Ejercicio 14 - Clase 07/Nombres","Nombre");// Cuidado la Direccion

    this->crearpopup();
}
bool Widget::setdb(QString arc, QString column)
{
    if(db.conectar(arc))
    {
        Archivo=arc;
...
void Widget::setList(QString column)
{
    columna=column;
    list.clear();
    QVector<QStringList>vec=db.select(column);
