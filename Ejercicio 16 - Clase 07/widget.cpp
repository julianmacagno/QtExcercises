    ...
    ui(new Ui::Widget)
{
    ...
    archivo = "../Ejercicio16-Clase07/Nombres.txt";
    this->setDiccionario(archivo);
...
void Widget::sugerencia()
{
    ...
    for (int i = 0, contador= 0; i < list.size()&& contador< 5; i++)
    {
        ...
        if(sugerencia.contains(line.toLower()) || sugerencia.contains(line.toUpper()) )
        {

            if(sugerencia.contains(" ")){
            QStringList parecidos;
            parecidos<<sugerencia.split(" ")<<sugerencia.split(" ").at(1);
            if(sugerencia.at(0).toLower()==line.toLower()){
                coincidencias.push_front(parecidos);
            }
            else{
...
void Widget::setDiccionario(QString archivo)
{
    QFile file(archivo);
